...
class LazySupervisedDataset(Dataset):
    ...
    def load_audio(self, audio_file):
        ...
        audio_process_values = self.audio_processor(audio_values, sampling_rate=16000, return_tensors="pt")
    ...
    def audio_get_item(self, data_item) -> Dict[str, torch.Tensor]:
        ...
        ret = preprocess_function(
            ...
            num_audio_tokens=audio_input['audio_token_num'],
            text_only=False,
            group_by_length=self.group_by_length,
        ...
        # 4. Создаем пустой image tensor
        empty_image = Image.new('RGB', (224, 224), (255, 255, 255))
        transform = self.get_transform()
        images = dynamic_preprocess(empty_image, min_num=self.min_dynamic_patch, max_num=1,
                                image_size=self.image_size, use_thumbnail=self.use_thumbnail)
        pixel_values = [transform(image) for image in images]
        pixel_values = torch.stack(pixel_values)
        ...
        item = dict(
            ...
            attention_mask=ret['attention_mask'][0],
            position_ids=position_ids[0],
            pixel_values=pixel_values,  # Теперь используем пустое изображение вместо None
    ...
    def multi_audio_get_item(self, data_item) -> Dict[str, torch.Tensor]:
        ...
        audio_inputs = []         # Для хранения информации по каждому аудио
        audio_token_blocks = []   # Для хранения строки с блоком токенов для каждого аудио
        all_audio_values = []     # Для аудио-признаков
        audio_len_after_cnns = [] # Для индивидуальных длин после CNN
        audio_token_nums = []     # Для количества аудио токенов (скалярное значение)
        ...
        for audio_file in audio_files:
            ...
            all_audio_values.append(audio_input['audio_values'])
            audio_len_after_cnns.append(audio_input['audio_len_after_cnn'])
            # Приводим audio_token_num к скалярному значению: если тензор содержит более одного элемента, берем первый
            token_num_tensor = audio_input['audio_token_num'].squeeze()
            token_num = int(token_num_tensor.flatten()[0].item())
            audio_token_nums.append(token_num)
            # Формируем отдельный блок токенов для этого аудио
            block = f'{AUDIO_START_TOKEN}{AUDIO_CONTEXT_TOKEN * token_num}{AUDIO_END_TOKEN}'
            audio_token_blocks.append(block)

        # Вместо конкатенации по оси времени stack аудио признаки, так каждый аудио обрабатывается отдельно.
        # Получим тензор формы [num_audios, C, T]
        combined_audio_values = torch.stack(all_audio_values, dim=0)
        ...
        # Заменяем первую встречу плейсхолдера <audio> на сформированную строку
        ...
        item = dict(
            ...
            attention_mask=ret['attention_mask'][0],
            position_ids=position_ids[0],
            pixel_values=pixel_values,  # Dummy image(s)
            image_flags=torch.tensor([0], dtype=torch.long),
            audio_values=combined_audio_values,       # Тензор формы [num_audios, C, T]
            audio_len_after_cnn=torch.tensor(audio_len_after_cnns, dtype=torch.long),  # Тензор [num_audios]
            audio_token_num=torch.tensor(audio_token_nums, dtype=torch.long),  # Тензор [num_audios]
            audio_flags=torch.ones(len(audio_inputs), dtype=torch.long)  # Тензор [num_audios]
        )
        return item
