# TODO in preprocess we need to think how to process audio correctly.
import io
...
def preprocess_internlm(
    ...
    if not text_only:
        ...
        for conversation in conversations:
            ...
            for i in range(num_image):
                image_tokens = f'{IMG_START_TOKEN}{IMG_CONTEXT_TOKEN * num_image_token_list[i]}{IMG_END_TOKEN}'
                conversation = conversation.replace('<image>', image_tokens, 1)
            # Обработка аудио (если задано)
            if num_audio_tokens is not None and '<audio>' in conversation:
                audio_tokens = f'{AUDIO_START_TOKEN}{AUDIO_CONTEXT_TOKEN * num_audio_tokens}{AUDIO_END_TOKEN}'
    ...
    # Дополнительная логика для маскирования (как есть)
    for conversation, target in zip(conversations, targets):
        total_len = int(target.ne(tokenizer.pad_token_id).sum())
        cur_len = 1
        target[:cur_len] = IGNORE_TOKEN_ID
...
def preprocess_internomni(
    ...
    for conversation, target in zip(conversations, targets):
        total_len = int(target.ne(tokenizer.pad_token_id).sum())
        # TODO: check if this is correct if you set this to 1 it will give you an error
        cur_len = 2
        target[:cur_len] = IGNORE_TOKEN_ID
        parts = conversation.split(conv.roles[1])
        info = parts[0] + conv.roles[1]
