...
def pad_tensor_list(tensor_list, pad_value=0):
    """
    Принимает список тензоров с разной первой размерностью и дополняет их до max_len по первой оси,
    затем возвращает тензор размера (B, max_len, ...).
    """
    ...
    for t in tensor_list:
        ...
        if pad_size > 0:
            pad_tensor = torch.full((pad_size,) + t.shape[1:], pad_value, dtype=t.dtype, device=t.device)
            t_padded = torch.cat([t, pad_tensor], dim=0)
        else:
            t_padded = t
        padded.append(t_padded)
    return torch.stack(padded, dim=0)

def pad_list_of_scalars(scalar_list, pad_value=0, dtype=torch.long):
    """
    Принимает список скаляров (или 1D тензоров) и возвращает тензор размера (B, max_len),
    где max_len — максимальное число элементов среди сэмплов.
    """
    max_len = max(len(s) if isinstance(s, (list, np.ndarray)) else 1 for s in scalar_list)
    padded = []
    for s in scalar_list:
        if isinstance(s, (list, np.ndarray)):
            arr = torch.tensor(s, dtype=dtype)
        ...
        if pad_size > 0:
            pad_tensor = torch.full((pad_size,), pad_value, dtype=dtype, device=arr.device)
            arr = torch.cat([arr, pad_tensor], dim=0)
        padded.append(arr)
    return torch.stack(padded, dim=0)
...
def concat_pad_data_collator(features, max_item_length=None, pad_id=0):
    ...
    # Паддинг для последовательностей (input_ids, labels, attention_mask, position_ids, loss_weight)
    batch_lens = [feat['input_ids'].shape[0] for feat in features]
    ...
    # Здесь разделяем ключи, для которых нужно выполнить паддинг по первой размерности (сохранение группы сэмплов)
    special_keys = {'pixel_values', 'image_flags', 'audio_values', 'audio_len_after_cnn', 'audio_token_num', 'audio_flags'}
    for k, v in first.items():
        if k in ('label', 'label_ids'):
            continue
        if v is None or isinstance(v, str):
            continue

        if k in special_keys:
            # Для аудио и изображений выполняем паддинг по первой оси
            if k in {'audio_values', 'pixel_values'}:
                # Ожидаем, что f[k] — тензор с формой (num_tokens, ...) и число токенов может различаться
                tensor_list = [f[k] for f in features]
                batch[k] = pad_tensor_list(tensor_list, pad_value=0)
            else:
                # Для остальных специальных ключей (обычно 1D данные)
                scalar_list = [f[k] for f in features]
                batch[k] = pad_list_of_scalars(scalar_list, pad_value=0, dtype=torch.long)
        else:
            # Для остальных ключей, где все сэмплы имеют одинаковую форму, используем stack
            if isinstance(v, torch.Tensor):
...
from internvl.model.audio_whisper import WhisperConfig, WhisperModel, WhisperProcessor
...
class ModelArguments:
    ...
    freeze_audio_encoder: bool = field(
        default=False,
        metadata={'help': 'Set to True to freeze the audio encoder. Default is False.'},
    )
    freeze_mlp_vision: bool = field(
        default=False,
        metadata={'help': 'Set to True to freeze the MLP. Default is False.'},
    )
    freeze_mlp_audio: bool = field(
        default=False,
        metadata={'help': 'Set to True to freeze the MLP. Default is False.'},
    )
...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(
        ...
        logger.info(f'[Dataset] num_audio_token: {num_audio_token}')
    ...
    def get_preprocess_function(self):
        ...
        elif self.template_name == 'internlm2-chat': # our template
            preprocess_function = preprocess_internlm
        elif self.template_name == 'internomni':
            preprocess_function = preprocess_internomni
    ...
    def load_audio(self, audio_file):
        """
        Загружает аудиофайл и возвращает словарь:
        {
            "audio_values": тензор [C, T],
            "audio_len_after_cnn": int,
            "audio_token_num": int,
        }
        """
        audio_values, _ = librosa.load(audio_file, sr=16000)
        audio_process_values = self.audio_processor(
        audio_values, sampling_rate=16000, return_tensors="pt"
        )
        # По умолчанию WhisperProcessor возвращает [1, 128, T], убираем "batch dim"
        input_features = audio_process_values["input_features"].squeeze(0)  # -> [128, T]

        # Если audio_len_after_cnn и audio_token_num тоже тензоры размерности [1],
        # можно превратить их в обычные int:
        audio_len_after_cnn = int(audio_process_values["audio_len_after_cnn"].item())
        audio_token_num = int(audio_process_values["audio_token_num"].item())

        return {
            "audio_values": input_features,        # [128, T]
            "audio_len_after_cnn": audio_len_after_cnn,  # int
            "audio_token_num": audio_token_num,          # int
        }
    ...
    def get_audio_path(self, audio_path):
    ...
    def audio_get_item(self, data_item) -> Dict[str, torch.Tensor]:
        # 1. Получаем путь к аудиофайлу и загружаем аудио
        audio_file = data_item['audio']
        if not audio_file.endswith(('.wav', '.mp3')):
            raise ValueError(f"Unsupported audio format: {audio_file}")
        audio_path = os.path.join(self.root, audio_file)
        audio_input = self.load_audio(audio_path)
        
        # load_audio возвращает audio_values с формой [C, T]; приводим к [1, C, T]
        audio_values = audio_input["audio_values"].unsqueeze(0)  # [1, C, T]
        
        # 2. Обработка текстовой части: формируем токен блок для аудио
        token_num = audio_input["audio_token_num"]
        audio_token_block = f'{AUDIO_START_TOKEN}{AUDIO_CONTEXT_TOKEN * token_num}{AUDIO_END_TOKEN}'
        
        conversation = deepcopy(data_item['conversations'])
        if '<audio>' not in conversation[0]['value']:
            conversation[0]['value'] = '<audio>\n' + conversation[0]['value']
        conversation[0]['value'] = conversation[0]['value'].replace('<audio>', audio_token_block, 1)
        
        # 3. Препроцессинг текста с использованием выбранной функции
        preprocess_function = self.get_preprocess_function()
        ret = preprocess_function(
            ...
            num_audio_tokens=None,  # дополнительная замена не нужна, т.к. мы уже вставили токен блок
            text_only=False,
            group_by_length=self.group_by_length,
        ...
        # 4. Создаем dummy-изображение
        empty_image = Image.new('RGB', (224, 224), (255, 255, 255))
        transform = self.get_transform()
        # Вызываем dynamic_preprocess, но независимо от того, сколько патчей вернется, выбираем только первый
        ...
        if isinstance(images, list) and len(images) > 0:
            ...
            dummy_img = empty_image
        pixel_values = transform(dummy_img)  # Получаем тензор формы [3, 448, 448]
        
        # 5. Формируем итоговый словарь, оборачивая числовые значения в тензоры
        item = dict(
            input_ids = ret['input_ids'][0] if isinstance(ret['input_ids'][0], torch.Tensor)
                        else torch.tensor(ret['input_ids'][0]),
            labels = ret['labels'][0] if isinstance(ret['labels'][0], torch.Tensor)
                        else torch.tensor(ret['labels'][0]),
            attention_mask = ret['attention_mask'][0] if isinstance(ret['attention_mask'][0], torch.Tensor)
                        else torch.tensor(ret['attention_mask'][0]),
            position_ids = position_ids[0] if isinstance(position_ids[0], torch.Tensor)
                        else torch.tensor(position_ids[0]),
            pixel_values = pixel_values,  # теперь с формой [3, 448, 448]
            image_flags = torch.tensor([0], dtype=torch.long),
            # Аудио:
            audio_values = audio_values,  # [1, C, T]
            audio_len_after_cnn = torch.tensor([audio_input["audio_len_after_cnn"]], dtype=torch.long),
            audio_token_num = torch.tensor([audio_input["audio_token_num"]], dtype=torch.long),
            audio_flags = torch.tensor([1], dtype=torch.long)
        )
        return item


    def multi_audio_get_item(self, data_item) -> Dict[str, torch.Tensor]:
        # 1. Обработка каждого аудио по отдельности
        audio_files = data_item['audios']
        audio_values_list = []   # Для аудио-признаков [C, T]
        audio_len_list = []      # Для индивидуальных длин после CNN (int)
        audio_token_list = []    # Для количества аудио токенов (int)
        audio_token_blocks = []  # Для формирования строки с токенами
        ...
        for audio_file in audio_files:
            ...
            audio_path = os.path.join(self.root, audio_file)
            audio_input = self.load_audio(audio_path)  
            # load_audio() теперь гарантированно возвращает "audio_values" с формой [C, T]
            audio_values_list.append(audio_input['audio_values'])  # [C, T]
            audio_len_list.append(audio_input['audio_len_after_cnn'])  # int
            audio_token_list.append(audio_input['audio_token_num'])    # int

            # Формируем токены для этого аудио
            token_num = audio_input['audio_token_num']
            block = f'{AUDIO_START_TOKEN}{AUDIO_CONTEXT_TOKEN * token_num}{AUDIO_END_TOKEN}'
            audio_token_blocks.append(block)

        # Склеиваем аудиопризнаки в тензор формы [num_audios, C, T]
        combined_audio_values = torch.stack(audio_values_list, dim=0)

        # 2. Обработка текстовой части
        conversation = deepcopy(data_item['conversations'])
        if '<audio>' not in conversation[0]['value']:
            conversation[0]['value'] = '<audio>\n' + conversation[0]['value']
        # Собираем строку токенов для всех аудио
        combined_audio_tokens = ''.join(audio_token_blocks)
        conversation[0]['value'] = conversation[0]['value'].replace('<audio>', combined_audio_tokens, 1)
        ...
        ret = preprocess_function(
            ...
            self.tokenizer,
            num_image_token_list=[0],
            num_audio_tokens=None,  # дополнительная замена не нужна
            text_only=False,
            group_by_length=self.group_by_length,
            use_packed_ds=self.use_packed_ds,
            ds_name=self.ds_name
        ...
        # 4. Создаем dummy-изображение (как в случае с одним аудио)
        empty_image = Image.new('RGB', (224, 224), (255, 255, 255))
        transform = self.get_transform()
        images = dynamic_preprocess(
            empty_image,
        ...
        # 5. Формируем итоговый словарь, обеспечивая, что все значения — тензоры
        item = dict(
            input_ids = ret['input_ids'][0] if isinstance(ret['input_ids'][0], torch.Tensor) 
                        else torch.tensor(ret['input_ids'][0]),
            labels = ret['labels'][0] if isinstance(ret['labels'][0], torch.Tensor) 
            ...
            pixel_values = pixel_values,  # dummy image, уже тензор
            image_flags = torch.tensor([0], dtype=torch.long),
            # Аудио:
            audio_values = combined_audio_values,              # [num_audios, C, T]
            audio_len_after_cnn = torch.tensor(audio_len_list, dtype=torch.long),  # [num_audios]
            audio_token_num = torch.tensor(audio_token_list, dtype=torch.long),    # [num_audios]
            audio_flags = torch.ones(len(audio_files), dtype=torch.long)           # [num_audios]
        )
        return item
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]: # method for getting item from dataset
        ...
        while True:
            ...
            try:
                ...
                elif 'audios' in data_item and isinstance(data_item['audios'], list):
                    ret = self.multi_audio_get_item(data_item)
                elif 'audio' in data_item and data_item['audio'] is not None and data_item['audio'] != '':
...
def build_datasets(
    ...
    for ds_idx, ds_name in enumerate(ds_collections.keys()):
        ...
        dataset = LazySupervisedDataset(
            ...
            num_audio_token=model.num_audio_token,
...
def main():
    ...
    audio_processor = WhisperProcessor.from_pretrained(model_args.model_name_or_path, trust_remote_code=True)
    ...
    if model_args.model_name_or_path is not None:
        ...
        config = InternVLChatAudioConfig.from_pretrained(model_args.model_name_or_path)
        ...
        model = InternVLChatAudioModel.from_pretrained(
    ...
    if not hasattr(model, 'num_audio_token'):
        # Здесь используем максимальное значение, основываясь на том, что:
        # audio_token_num = (audio_len_after_cnn - 2) // 2 + 1
        # где audio_len_after_cnn для 30-секундного аудио примерно 1487
        model.num_audio_token = (1487 - 2) // 2 + 1  # ≈ 743
    ...
    train_dataset = build_datasets(
        data_args, tokenizer, audio_processor, tcs_loader, model, group_by_length=training_args.group_by_length,
